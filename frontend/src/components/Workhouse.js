...
class Workhouse extends Component {
  ...
  submitRegister(event) {
    ...
    return axios
      .post(baseUrl + "workhouse/register", {
  ...
  render() {
    if (this.props.Workhouses.isLoading) {
      ...
      const items = this.props.Workhouses.workhouses.map((item) => {
        return <RenderItem workhouses={item} key={item.w_id} />;
      });
      ...
      return (
        <div className="container-fluid">
          ...
          >
            <ModalHeader toggle={this.toggleModalRegister}>
              Register Workhouse
            </ModalHeader>
            <ModalBody>
                ...
                <form className="form-inline">
                  <div className="form-group ml-3">
                    <input
                      ...
                      onChange={this.changeSearchedItem}
                      placeholder="Search by Index no"
                    />
                  </div>
                  <Link to={`workhouse/${this.state.searchedItem}`}>
